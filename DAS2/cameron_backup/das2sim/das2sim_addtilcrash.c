#define S_FUNCTION_NAME das2sim_addtilcrash
#define S_FUNCTION_LEVEL 2
/* ABSTRACT: This is a test function written by Cameron LaMack on 4/16/21. The purpose
of this tester function is to determine whether the trouble with the simulink block is a fault of
Simulink or this function. So the way to determine that is to remove all references to das2 to sdbio,
replace them with blank dummy variables, and then see if we are able to run a simulink model with those
dummy variables.*/
...
static void mdlInitializeSizes(SimStruct *S)
{
	...
	ssSetNumSFcnParams(S, 0); //this is a cameron change !!!!!!!!!!!!!!!NOTE
							  //ssSetSFcnParamTunable(S, 0, 0); SETTING ssSetNumSFcnParams(S, 0) = ssSetNumSFcnParams(S, 1) gives an error
	...
	//else {
	num_inputs = 1;
	//}/*it is set to 0 in line 31, so this should be = 1*/

	if (ssGetNumSFcnParams(S) != ssGetSFcnParamsCount(S)) { //this line added by cameron
...
static void mdlInitializeConditions(SimStruct *S)
{
	...
	/*note for future cameron and others, these are initial velocities (probably)*/
	xC[5] = 0;
	xC[6] = 0;
	xC[7] = 0;
	xC[8] = 0;
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
	...
	for (i = 0; i < NDOF; i++)
	{
		xx[i] = xC[i]; /* write states to xx */
	}
}



#define MDL_DERIVATIVES  /* Change to #undef to remove function or #define to define */
...
{
	/*SDBIO_terminate();*/
	//UNUSED_ARG(S); /*this is a cameron additon*/
}
