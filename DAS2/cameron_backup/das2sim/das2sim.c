...
static void mdlInitializeSizes(SimStruct *S)
{
   ...
   if (SDBIO_init(filename,error_msg))
   {
	...
	ssSetNumSFcnParams(S, 1); //changed from ssSetNumSFcnParams(S, 1);
	//ssSetSFcnParamTunable(S,0,0); this and the previous line are a cameron change
...
static void mdlInitializeSampleTimes(SimStruct *S)
{
	...
	ssSetSampleTime(S, 1, 0.001); // Cameron changed from 0.02 to match sdbio
