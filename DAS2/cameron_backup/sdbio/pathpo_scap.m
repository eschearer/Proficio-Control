function error = pathpo_scap(path, modelname)

% pathpo_scap.m
%
% Computes best fitting polynomial for scapular muscle force vectors as a function
% of kinematic degrees of freedom q.
%
% This is done from scapular force vectors generated by SIMM
%
% Dimitra Blana - 7/23/08
% Based on pathpoly.m by Ton van den Bogert
%
% notes:
% - writes all polynomials to one file (poly_results_scap.txt)
...
% results file (output)
fullpath = [folder '/poly_results_scap.txt'];
...
%% get scapular force vectors out of SIMM motion file
...
while d==0
    ...
    s = fgets(configfile);
    d = sscanf(s,'%d');
    % this is 1 if the muscle attaches on the scapula, 0 if it doesn't
end
fgets(configfile);
...
% put all the fvector data in matrix scapfvector
scapfvector = fscanf(fvectorfile,'%f',[num_col,inf]); % it reads in column order
scapfvector = scapfvector';
num_data = size(scapfvector,1);
fclose(fvectorfile);
...
while 1
    ...
    % scapular fvector data for this muscle come from ndofs files, with names
    ...
    for i = 1:3  % three components of the fvector (x-y-z)
        ...
        % read data from scapfvector matrix
        % take fvector and angles from the right columns
        b = scapfvector(:,mcol);
        ang = (scapfvector(:,acol) + 1e-6) * pi/180;	% convert to radians and protect against angle = 0.0
